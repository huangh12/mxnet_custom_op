...
struct BoxNMSParam : public dmlc::Parameter<BoxNMSParam> {
  ...
  DMLC_DECLARE_PARAMETER(BoxNMSParam) {
    ...
    .describe("The output box encoding type. \n"
        " \"corner\" means boxes are encoded as [xmin, ymin, xmax, ymax],"
        " \"center\" means boxes are encodes as [x, y, width, height].");
    DMLC_DECLARE_FIELD(legacy_box).set_default(false)
    .describe("Whether to calculate the width or height in legacy way.");
  }
};  // BoxNMSParam
...
struct corner_to_center {
  ...
  MSHADOW_XINLINE static void Map(int i, DType *data, int stride, bool legacy_box) {
    float offset = legacy_box? 1:0;
    int index = i * stride;
    ...
    data[index+2] = right - left + offset;
    data[index+3] = bot - top + offset;
  }
};
...
struct center_to_corner {
  ...
  MSHADOW_XINLINE static void Map(int i, DType *data, int stride, bool legacy_box) {
    ...
    data[index] = x - width + offset/2;
    data[index+1] = y - height + offset/2;
    data[index+2] = x + width - offset/2;
    data[index+3] = y + height - offset/2;
  }
};

template<typename DType>
MSHADOW_XINLINE DType BoxArea(const DType *box, int encode, bool legacy_box) {
  DType a1 = box[0];
  DType a2 = box[1];
  DType a3 = box[2];
  DType a4 = box[3];
...
struct compute_area {
  ...
  MSHADOW_XINLINE static void Map(int i, DType *out, const DType *in,
                                  ...
                                  int topk, int num_elem, int stride, int encode, bool legacy_box) {
    int b = i / topk;
    int k = i % topk;
    ...
    out[index] = BoxArea(in + in_index, encode, legacy_box);
  }
};
...
void NMSApply(mshadow::Stream<cpu> *s,
  ...
  for (int ref = 0; ref < topk; ++ref) {
    ...
    Kernel<nms_impl, cpu>::Launch(s, num_batch * num_worker,
      ...
      threshold, force_suppress, in_format, legacy_box);
  }
}
...
void BoxNMSForward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    ...
    Kernel<compute_area, xpu>::Launch(s, num_batch * topk,
     ...
     topk, num_elem, width_elem, param.in_format, param.legacy_box);
    ...
    mxnet::op::NMSApply(s, num_batch, topk, &sorted_index,
                        ...
                        param.force_suppress, param.in_format, param.legacy_box);
    ...
    if (param.in_format != param.out_format) {
      if (box_common_enum::kCenter == param.out_format) {
        Kernel<corner_to_center, xpu>::Launch(s, num_batch * num_elem,
          out.dptr_ + coord_start, width_elem, param.legacy_box);
      } else {
...
struct BoxOverlapParam : public dmlc::Parameter<BoxOverlapParam> {
  ...
  DMLC_DECLARE_PARAMETER(BoxOverlapParam) {
    ...
    .describe("The box encoding type. \n"
        " \"corner\" means boxes are encoded as [xmin, ymin, xmax, ymax],"
        " \"center\" means boxes are encodes as [x, y, width, height].");
    DMLC_DECLARE_FIELD(ovr_type).set_default(overlap_type_enum::kIoU)
    .add_enum("iou", overlap_type_enum::kIoU)
    .add_enum("ioa", overlap_type_enum::kIoA)
    .describe("The box overlap type. \n"
        " \"iou\" means the default iou that inter_area/union_area,"
        " \"ioa\" means the overlap is inter_area/first_box_area.");
    DMLC_DECLARE_FIELD(legacy_box).set_default(false)
    .describe("Whether to calculate the width or height in legacy way.");
  }
};  // BoxOverlapParam
...
struct compute_overlap {
  ...
  MSHADOW_XINLINE static void Map(int i, DType *out, const DType *lhs,
                                  ...
                                  int begin, int stride, int encode, bool legacy_box,
                                  int ovr_type) {
    int l = i / num;
    int r = i % num;
    int l_index = l * stride + begin;
    int r_index = r * stride + begin;
    DType intersect = Intersect(lhs + l_index, rhs + r_index, encode, legacy_box);
    intersect *= Intersect(lhs + l_index + 1, rhs + r_index + 1, encode, legacy_box);
    if (intersect <= 0) {
      out[i] = DType(0);
      return;
    }
    DType l_area = BoxArea(lhs + l_index, encode, legacy_box);
    DType r_area = BoxArea(rhs + r_index, encode, legacy_box);
    if (overlap_type_enum::kIoU == ovr_type) {
      out[i] = intersect / (l_area + r_area - intersect);
    } else {
      out[i] = intersect / (l_area);
    }
  }
};
...
void BoxOverlapForward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    ...
    Kernel<compute_overlap, xpu>::Launch(s, lsize * rsize, out.dptr_,
     lhs.dptr_, rhs.dptr_, rsize, 0, 4, param.format, param.legacy_box, param.ovr_type);
  });
}
