...
template<typename DType>
MSHADOW_XINLINE DType Intersect2(const DType *a, const DType b1, const DType b2, int encode, bool legacy_box) {
  const DType a1 = a[0];
  const DType a2 = a[2];
  DType left, right;
  ...
  if (box_common_enum::kCorner == encode) {
    ...
    const DType aw = a2 / 2;
    const DType bw = b2 / 2;
    const DType al = a1 - aw + offset/2;
    const DType ar = a1 + aw - offset/2;
    const DType bl = b1 - bw + offset/2;
    const DType br = b1 + bw - offset/2;
    left = bl > al ? bl : al;
    right = br < ar ? br : ar;
  }
  const DType w = right - left + offset;
...
__global__ void nms_apply_kernel(const int topk, int32_t *index,
                                 ...
                                 const int encode, const int start_offset, const bool legacy_box) {
    ...
    if (ref_area_offset >= 0) {
      ...
      for (int i = 0; i < N; ++i) {
        ...
        if (my_pos > ref && my_pos < end && s_index[my_pos] >= 0) {
          ...
          DType intersect = Intersect(input + ref_offset, input + pos_offset, encode, legacy_box);
          intersect *= Intersect(input + ref_offset + 1, input + pos_offset + 1, encode, legacy_box);
...
__global__ void nms_apply_kernel_rest(const int topk, int32_t *index,
                                 ...
                                 const int blocks_per_batch, const bool legacy_box) {
  constexpr int block_size = 512;
...
#pragma unroll
    ...
    for (int ref = start_ref; ref < start_ref + N * block_size; ++ref) {
      ...
      if (ref_area_offset >= 0) {
        ...
        DType intersect = Intersect2(input + ref_offset, my_box[0], my_box[2], encode, legacy_box);
        intersect *= Intersect2(input + ref_offset + 1, my_box[1], my_box[3], encode, legacy_box);
        const DType iou = intersect /
