...
namespace overlap_type_enum {
enum OverlapType {kIoU, kIoA};
}
// compute line intersect along either height or width
template<typename DType>
MSHADOW_XINLINE DType Intersect(const DType *a, const DType *b, int encode, bool legacy_box) {
  DType a1 = a[0];
  DType a2 = a[2];
  DType b1 = b[0];
  DType b2 = b[2];
  ...
  if (box_common_enum::kCorner == encode) {
    ...
    DType aw = a2 / 2;
    DType bw = b2 / 2;
    DType al = a1 - aw + offset/2;
    DType ar = a1 + aw - offset/2;
    DType bl = b1 - bw + offset/2;
    DType br = b1 + bw - offset/2;
...
/*!
   ...
   * \param legacy_box whether add 1 when calculating width of height of the box
...
struct nms_impl {
  ...
  MSHADOW_XINLINE static void Map(int i, int32_t *index, const int32_t *batch_start,
                                  ...
                                  float thresh, bool force, int encode, bool legacy_box) {
    int b = i / k;  // batch
