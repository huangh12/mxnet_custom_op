...
struct TopKParam : public dmlc::Parameter<TopKParam> {
  ...
  DMLC_DECLARE_PARAMETER(TopKParam) {
    ...
    DMLC_DECLARE_FIELD(do_sort).set_default(true)
      .describe("Whether to sort the k elements or keep the original relative order.");                
  ...
  for (int i = 0; i < M; ++i) {
    ...
    if (is_ascend) {
      if (full_sort) {
        ...
        if (!do_sort) std::sort(indices, indices+K);
      } else {
...
MSHADOW_FORCE_INLINE void TopKSort(const Tensor<gpu, 1, DType>& dat,
  ...
  if (!do_sort) {
    ...
    Tensor<gpu, 1, char> sort_work(work.dptr_+id_size, Shape1(work.size(0)-id_size), s);
    for (int i = 0; i < M; ++i) {
      Tensor<gpu, 1, int> values(reinterpret_cast<int*>(dat.dptr_+N*i), Shape1(K), s);
      Tensor<gpu, 1, int> indices(reinterpret_cast<int*>(ind.dptr_+N*i), Shape1(K), s);
      mxnet::op::SortByKey(indices, values, true, &sort_work);
    }
  }
}
...
void TopKImpl(const RunContext &ctx,
  ...
  TopKSort(sorted_dat, indices, temp_workspace, k, element_num, is_ascend, param.do_sort, s);
